...
async function bucketSort(array){
    ...
    array.forEach(val=>{
        buckets[Math.floor((val-min)/n)+1].push(val);
    })
    ...
    for(let i=0;i<n;i++)
    {
        ...
        for(let j=0;j<buckets[i].length;j++)
        {
            ...
            array[ind]=buckets[i][j];
            bars[ind].style.height=array[ind]*heightFactor + "px";
            bars[ind].style.backgroundColor="red";
            ind++;
            await sleep(speedFactor);
        }
    }
...
async function radixSort(array){
    ...
    while(divisor<maxNum)
    {
        ...
        {
            bucket[Math.floor((num%divisor)/(divisor/10))].push(num);
        }

        array=[].concat.apply([],bucket);
        divisor*=10;
    }
...
async function countSort(array){
    ...
    //Count Array
    var count=Array.from({length:max},(_,i)=>0);
